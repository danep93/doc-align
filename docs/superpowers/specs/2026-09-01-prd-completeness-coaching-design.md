...
**This is a deliberate departure from this project's earlier "privacy-first, never analyze document content" positioning.** That framing has been explicitly deprioritized in favor of hitting product goals (see `CLAUDE.md` and project memory `project_privacy_priority_shift`). The invariant that survives is narrower: document content is still never *persisted* wholesale — only the two extracted snippets (Press Release / DoD text) are stored here, the same way `changeSummary` already stores section-level diff metadata without the full document.

**This will grow in later phases, by design.** Converting a doc to a Linear project and then tracking drift against it (future specs, not built here) will need more than these two snippets — the baseline fields carried into the Linear project (once decided: things like project definition, Definition of Done, north star metric, IC-weeks estimate) have to be stored so that, later, "what the Linear project turned into" can be compared against "what the doc originally said" to surface inaccuracy or scope drift. That comparison is impossible without persisting the baseline extracted content it's diffed against. This spec's two-field storage is the first, narrowest instance of that pattern — not an exception to be revisited later.
